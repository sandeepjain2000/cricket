#!/usr/bin/env python3
"""
Cricket T20 & LOI Downloader - DEBUG VERSION
Logs detailed structure information to identify why deliveries aren't being inserted
"""
...
SCRIPT_NAME = "cricket_t20_loi_debug.py"
VERSION = "4.0.0"
...
log(f"  T20 & LOI Downloader (DEBUG VERSION)")
...
stats = {'matches_inserted': 0, 'deliveries_inserted': 0, 'errors': 0, 'debug': 0}
debug_logged = False

def process_format(format_name, db_path, zip_url):
    global stats, debug_logged
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            log(f"✓ Initialized database: {db_path} (recreated with DROP TABLE)")
            ...
            for file_idx, json_file in enumerate(sorted(json_files), 1):
                try:
                    ...
                    # DEBUG: Log structure of first few files
                    if not debug_logged and file_idx <= 3:
                        log(f"\n  [DEBUG] File {file_idx}: {json_file.name}")
                        log(f"    Top-level keys: {list(match_data.keys())}")
                        innings = match_data.get("innings", [])
                        log(f"    Number of innings: {len(innings)}")

                        if innings:
                            first_inning = innings[0]
                            log(f"    Inning 1 keys: {list(first_inning.keys())}")
                            overs = first_inning.get("overs", [])
                            log(f"    Number of overs in inning 1: {len(overs)}")

                            if overs:
                                first_over = overs[0]
                                log(f"    Over 1 keys: {list(first_over.keys())}")
                                deliveries = first_over.get("deliveries", [])
                                log(f"    Number of deliveries in over 1: {len(deliveries)}")
                                if deliveries:
                                    log(f"    First delivery keys: {list(deliveries[0].keys())}")
                        log("")
